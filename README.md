...
UPDATE 22-May-2026: The install script has been updated to support Omarchy 3.0+ out of the box. 
...
4. Secure Boot: **Debes deshabilitar Secure Boot en la BIOS** antes de instalar los drivers NVIDIA. El driver propietario NVIDIA no cargará con Secure Boot activado a menos que firmes manualmente el módulo del kernel. En laptops Lenovo (Ideapad Gaming 3, etc.) Secure Boot viene habilitado por defecto. Entra a la BIOS (F2 al iniciar) y desactívalo en la sección Security > Secure Boot.

5. Gráficos Híbridos (laptops): Si tu laptop tiene gráficos Intel/AMD integrados + NVIDIA discreta (como la mayoría de laptops gaming), el script configurará NVIDIA como GPU por defecto. Esto puede aumentar el consumo de batería. Si prefieres usar solo la GPU integrada para ahorrar batería, puedes instalar `envycontrol` después:
   ```
   yay -S envycontrol
   sudo envycontrol -s integrated
   ```

6. This script now automatically handles NVIDIA driver installation by enforcing the proprietary 580xx drivers (via CachyOS `chwd`). This is necessary to avoid known regressions with hardware video decoding and browser flickering present in the newer open-kernel module drivers. El script también:
   - Configura los parámetros del kernel `nvidia_drm.modeset=1` y `nvidia_drm.fbdev=1` en GRUB
   - Agrega los módulos NVIDIA al initramfs
   - Verifica que el archivo de IDs de chwd exista antes de parchearlo
   - Detecta Secure Boot y advierte al usuario
...
### Paso 0: Preparación (IMPORTANTE para laptops con NVIDIA)

1. **Deshabilita Secure Boot en la BIOS** (F2 al iniciar → Security → Secure Boot → Disabled)
2. **Identifica tu GPU NVIDIA**:
   ```bash
   lspci -nn -d 10de:
   ```
   Anota el ID entre `[10de:XXXX]` (ej: `1f99` para GTX 1650, `25a0` para RTX 3050).
3. **Asegúrate de tener los paquetes base**:
   ```bash
...
### Paso 2: Después de la instalación

1. **Reinicia el sistema** para que los módulos NVIDIA se carguen correctamente.
2. **Verifica que NVIDIA funciona**:
   ```bash
   nvidia-smi
   lsmod | grep nvidia
   ```
3. **Si ves pantalla negra al arrancar**, arranca desde el kernel anterior en GRUB (Advanced Options) y revisa los logs:
   ```bash
   sudo journalctl -b -1 | grep -i nvidia
   ```
4. **Para optimizar batería en laptop** (gráficos híbridos):
   ```bash
   yay -S envycontrol
   sudo envycontrol -s integrated  # usa solo Intel/AMD
   # o
   sudo envycontrol -s hybrid       # modo híbrido (elige GPU por app)
   ```
