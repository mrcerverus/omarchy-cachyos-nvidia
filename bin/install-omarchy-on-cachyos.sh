...
# ─── Safety checks ───
if [ "$(id -u)" = 0 ]; then
    echo "Error: No ejecutes este script como root. Ejecútalo como un usuario normal."
    exit 1
fi

if ! command -v git &> /dev/null; then
    echo "Error: git no está instalado. Instálalo antes de ejecutar:"
...
if [ -d "$OMARCHY_DIR" ]; then
    echo "El directorio $OMARCHY_DIR ya existe. Actualizando..."
    cd "$OMARCHY_DIR" && git pull && cd -
else
    if ! git clone https://www.github.com/basecamp/omarchy "$OMARCHY_DIR"; then
        echo "Error: Falló la clonación del repositorio Omarchy."
        exit 1
    fi
fi

echo "Repositorio Omarchy listo."
